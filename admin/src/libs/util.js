...
export const setAdminId = (adminId) => {
  ...
  Cookies.set('adminId', adminId, { expires: cookieExpires || 1 })
}
...
//   let cookieExpires
//   if (getToken()) cookieExpires = getExpires(getToken())
//   Cookies.set('authMenu', authMenu, { expires: cookieExpires || 1 })
// }
...
export const setUserName = (username) => {
  let cookieExpires
  if (getToken()) cookieExpires = getExpires(getToken())
  Cookies.set('username', username, { expires: cookieExpires || 1 })
}
...
export const testfastLotteryInfo = (type) => {
  ...
  if (number === 12 || number === 13 || number === 3630) { // 快速赛车
    return {
      maxNum: 11,
      count: 10,
      repeat: false
    }
  } else if (number === 16 || number === 17) { // 快速时时彩
    return {
      maxNum: 10,
      count: 5,
      repeat: false
    }
  } else if (number === 18 || number === 19) { // 快速6合彩
    return {
      maxNum: 50,
      count: 7,
      repeat: false
