<template>
  <div>
    <Form ref="formKey" :model="formKey" :rules="rules" :label-width="100">
      <FormItem label="代理账号">
        <Input v-model="formKey.userName?formKey.userName:formData.userName" disabled class="w260x"/>
      </FormItem>
      <FormItem label="代理类型">
        <Input v-model="formKey.agent_type ? formKey.agent_type : formData.agent_type " disabled class="w260x"/>
      </FormItem>
  ...
  methods: {
    ...
    getData(data) {
      agencyDomainDetails(data).then(res => {
        ...
        this.formData.agent_type = level > 0 ? `${level}级代理` : "大股东"   
