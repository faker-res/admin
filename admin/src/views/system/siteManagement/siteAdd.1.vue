<template>
  <div>
    ...
    <Form :model="siteInfo.siteConfig.limit" ref="limit" :rules="otherRules" :label-width="130">
      <div class="clearfix bt">
        <p class="blue-font-nc mt20 mb10">站点设置</p>
        <!-- 服务器下拉框 -->
        <Form :label-width="130">
          <FormItem label="服务器" class="fl w45" prop="dbid">
        ...
        <FormItem label="网站维护" class="fl w100 bt mt20 pre">
          <div class="mt20">
            ...
            <span class="pl10 pr10" v-show="switchStatus">开始日期</span>
            <DatePicker
              v-show="switchStatus"
              type="datetime"
            ...
            ></DatePicker>
            <span class="pl30 pr10" v-show="switchStatus">结束日期</span>
            <DatePicker
              v-show="switchStatus"
              type="datetime"
              format="yyyy-MM-dd HH:mm"
            ...
            ></DatePicker> -->
            <Switch v-model="siteInfo.siteConfig.close.newStatus"></Switch>
            <span class="pl10 pr10" v-show="siteInfo.siteConfig.close.newStatus">开始日期</span>
            <DatePicker
              v-show="siteInfo.siteConfig.close.newStatus"
              type="datetime"
              format="yyyy-MM-dd HH:mm"
              class="w260x"
              placeholder="请选择日期"
              v-model="siteInfo.siteConfig.close.timeStart"
            ></DatePicker>
            <span class="pl30 pr10" v-show="siteInfo.siteConfig.close.newStatus">结束日期</span>
            <DatePicker
              v-show="siteInfo.siteConfig.close.newStatus"
              type="datetime"
              format="yyyy-MM-dd HH:mm"
          ...
          <div v-show="siteInfo.siteConfig.close.newStatus">
            <Input
              style="width: 70%;"
              class="mt20"
              type="textarea"
        ...
        <FormItem label="系统配置" class="fl w100 pre">
          <div class="mt20">
            ...
            <div class="mt20 clearfix" v-show="sysBtn != 'other'">
              <div v-show="sysBtn == 'android'" class="clearfix">
                <FormItem label="版本号" class="fl w40">
                  <Input class="w160x" v-model="siteInfo.siteConfig.Android.version"/>
                </FormItem>
                <FormItem label="下载链接" class="fl w40">
                  <Input class="w160x" v-model="siteInfo.siteConfig.Android.link"/>
                </FormItem>
                <FormItem label="提示文字" class="fl w40">
                  <Input class="w160x" v-model="siteInfo.siteConfig.Android.tip"/>
                </FormItem>
                <FormItem label="更新类型" class="fl w40">
                  <RadioGroup v-model="siteInfo.siteConfig.Android.type" class="type">
                    <Radio label="yes">
                      <span>强制更新</span>
                    </Radio>
                    <Radio label="no">
              ...
              <div v-show="sysBtn == 'ios'" class="clearfix">
                <FormItem label="版本号" class="fl w45">
                  <Input class="w160x" v-model="siteInfo.siteConfig.IOS.version"/>
                </FormItem>
                <FormItem label="下载链接" class="fl w45">
                  <Input class="w160x" v-model="siteInfo.siteConfig.IOS.link"/>
                </FormItem>
                <FormItem label="提示文字" class="fl w45">
                  <Input class="w160x" v-model="siteInfo.siteConfig.IOS.tip"/>
                </FormItem>
                <FormItem label="更新类型" class="fl w45">
                  <RadioGroup v-model="siteInfo.siteConfig.IOS.type" class="type">
                    <Radio label="yes">
                      <span>强制更新</span>
                    </Radio>
                    <Radio label="no">
...
export default {
  ...
  methods: {
    ...
    getDetailData(data) {
      siteManagementDetail(data).then(res => {
        ...
        if(!this.siteInfo.siteConfig.banner_pic_size){
          this.siteInfo.siteConfig.banner_pic_size = { banner_app: '', banner_app: ''}
        }
        let { close, service } = this.siteInfo.siteConfig;
        service.forEach(item => (item.show = true));
        close.newStatus = close.status == 'on' ? true : false
                // if(close.status == 'on'){
        //   close.newStatus = close.status == 'on' ? true : false
        //   // close.status = close.status == "on" ? true : false; //正常数据
        // }else {
          
        // }
        //处理后台新增的属性为null的情况
        if(!this.siteInfo.siteConfig.activity_pic_size) {
          this.siteInfo.siteConfig.activity_pic_size = { activity_pc: '', activity_app: ''}
        }
    ...
    submit() {
      ...
      data.close.status = data.close.newStatus ? 'on' : 'off'
